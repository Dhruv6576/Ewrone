-- Migration: 20260918000014_restore_late_capture_and_catalog_cleanup.sql
-- Description: Restore migration 10 late-capture handling with expires_at clearing on happy path,
--              archive M6 Arena, update The Dugout slug, and ensure tracked idempotent venue catalog.

-- 1. Restore private.confirm_booking_payment to Migration 10 parity + clear expires_at
...
-- 2. Catalog Cleanup (F5):
-- (a) Archive M6 Arena per reviewer finding (it has real bookings, so soft-archive)
update public.turfs
set archived_at = coalesce(archived_at, now())
where id = 'c3333333-cccc-cccc-cccc-cccccccccccc';

-- (b) Update The Dugout Box Cricket slug to a clean, human-readable slug
update public.turfs
set slug = 'the-dugout-indiranagar'
where id = '21efb282-8b54-47c2-88bc-987d9c0e208d'
  and slug = 'live-rzp-turf-1789761873594';
